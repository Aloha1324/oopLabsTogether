...
public class FrameworkPerformanceComparisonService {
    ...
    private void testReadOperations(PerformanceResults results) {
        ...
        if (!users.isEmpty()) {
            Optional<User> user = userService.getUserById(users.get(0).getId());
        }
        ...
        if (!functions.isEmpty()) {
            Optional<Function> function = functionService.findFunctionById(functions.get(0).getId());
        }
    ...
    private void testSearchOperations(PerformanceResults results) {
        ...
        List<User> users = userService.getAllUsers();
        if (!users.isEmpty()) {
            userService.getUserByUsername(users.get(0).getUsername());
        }
        long endTime = System.nanoTime();
